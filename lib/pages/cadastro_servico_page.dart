...
class _CadastroServicoPageState extends State<CadastroServicoPage> with TickerProviderStateMixin {
  ...
  Future<void> _filtrarServicos() async {
    ...
    try {
      final resultado = await ServicoService.buscarPaginado(query, _currentPage, size: _pageSize);
  ...
  Future<void> _carregarServicos() async {
    setState(() => _isLoadingServicos = true);
    try {
      final resultado = await ServicoService.buscarPaginado('', 0, size: _pageSize);
                        ...
                        Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              const SizedBox(width: 6),
                              Text(
                                'Cadastrado: ${DateFormat('dd/MM/yyyy').format(servico.createdAt!)}',
                                style: TextStyle(
                                  color: Colors.grey[600],
                                  fontSize: 10,
                                  fontWeight: FontWeight.w500,
